...
{{--      //分页--}}
    ...
    <script>
      ...
      /*用户-删除*/
      function member_del(obj,id){//obj代表当前元素
          layer.confirm('确认要删除吗？',function(index){
            $.post('/admin/user/'+id,{"_method":"delete","_token":"{{csrf_token()}}"},function (data) {
              console.log(data);
              if(data.status == 0){//如果删除成功
                $(obj).parents("tr").remove();//移除当前元素
                layer.msg(data.message,{icon:6,time:1000});//发送提示信息
              }else{
                layer.msg(data.message,{icon:5,time:1000});//发送提示信息
      ...
      function delAll (argument) {
        ...
        layer.confirm('确认要删除吗？'+data,function(index){
          ...
          $.get('/admin/user/del',{'ids':ids},function (data) {
            if(data.status == 0){//如果删除成功
              $(".layui-form-checked").not('.header').parents('tr').remove();//移除当前元素
              layer.msg(data.message,{icon:6,time:1000});//发送提示信息
            }else{
              layer.msg(data.message,{icon:5,time:1000});//发送提示信息
